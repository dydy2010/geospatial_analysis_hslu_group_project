...
output_file: "GEO_Project_Team3.pdf"
output:
  pdf_document:
    toc: true
    toc_depth: 1
...
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_file = "GEO_Project_Team3.pdf") })
---
\newpage
...
**Incomplete spatial coverage.** Five out of 35 quarters (Oberstrass, Alt-Wiedikon, Hirslanden, Gewerbeschule, and one unknown) have no traffic sensor data, meaning about 14% of Zurich cannot be analyzed with our method.

**Metric limitations.** Our data counts vehicles passing sensors, not congestion levels, so high-volume roads with good flow appear similar to congested ones. Additionally, we used January population snapshots to avoid double-counting, which means seasonal changes (students, tourists) are not captured.

**Interpretive constraints.** The Stress Index classification thresholds (+5% for Commuter Hub, -5% for Residential Zone) are interpretive choices, not statistically derived. Our team also lacks local Zurich knowledge, which would help explain patterns like construction projects, new developments, or policy changes that drive the spikes we observed.

Despite these limitations, the Stress Index provides a useful first look at which areas of Zurich face the biggest gap between population and traffic dynamics.
